...
def draw_game():
    try:
        ...
        if game_state.paused:
            ...
            # Duraklatma menüsü metni
            pause_text = font.render("OYUN DURAKLATILDI", True, WHITE)
            continue_text = font.render("Devam etmek için ESC'ye basın", True, WHITE)
            menu_text = font.render("Ana menüye dönmek için M'ye basın", True, WHITE)
        ...
        # Power-up efekt göstergeleri
        effect_y = 50
        for effect_type in power_up_manager.active_effects:
            effect_text = font.render(f"Aktif Güç: {effect_type}", True, WHITE)
...
def update_game():
    try:
        if not game_state.paused:
            # Top ve platform güncelleme
            if ball.active:
                ball.move()
                
                # Top ekranın altına düştü mü?
                if ball.y > screen_height:
                    ...
                    if game_state.lives <= 0:
                        ...
                        if game_state.is_logged_in():
                            db.update_high_score(game_state.current_user, game_state.score)
    ...
    try:
        ...
        if game_state.state == "game" and not game_state.paused:
            try:
                ...
                new_score = game_logic.check_collisions(ball, platform, block_manager, power_up_manager)
