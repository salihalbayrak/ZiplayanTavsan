...
class GameSettings:
    ...
    def draw_sound_settings(self, start_y):
        for i, (name, value) in enumerate(self.sound_settings.items()):
            # Slider başlığı
            text = self.text_font.render(name.replace("_", " ").title(), True, self.colors["text"])
            ...
            # Değer yüzdesi
            value_text = self.text_font.render(f"{int(value * 100)}%", True, self.colors["text"])
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if self.current_category == 0:
                content_y = self.screen_height * 0.3
                for i, key in enumerate(self.sound_settings.keys()):
                    ...
                    if slider_rect.collidepoint(mouse_pos):
                        ...
                        self.sound_settings[key] = max(0, min(1, value))
                        self.save_settings()
                        return None
